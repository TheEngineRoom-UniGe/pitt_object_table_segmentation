...
void initializeInputParameters( SupportSegmentation::Request  &req){
    minIterativeCloudPercentage = srvm::getServiceFloatParameter(
            req.min_iterative_cloud_percentual_size, DEFAULT_PARAM_SUPPORT_SRV_MIN_ITERATIVE_CLOUD_PERCENTAGE);
    minPlanePercentageSize = srvm::getServiceFloatParameter(
            req.min_iterative_plane_percentual_size, DEFAULT_PARAM_SUPPORT_SRV_MIN_ITERATIVE_SUPPORT_PERCENTAGE);
    maxVarianceThForHorizontal = srvm::getServiceFloatParameter(
            req.variance_threshold_for_horizontal, DEFAULT_PARAM_SUPPORT_SRV_VARIANCE_THRESHOLD_FOR_HORIZONTAL);
    minVarianceThForHorizontal = -1 * maxVarianceThForHorizontal;
    ransacThDistancePointShape = srvm::getServiceFloatParameter(
            req.ransac_distance_point_in_shape_threshold, DEFAULT_PARAM_SUPPORT_SRV_RANSAC_DISTANCE_POINT_IN_SHAPE_THRESHOLD);
